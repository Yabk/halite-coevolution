"""This code is from the following github repo: https://github.com/viewlagoon/HaliteRaineForce
...
DISTANCE_TO_PREFERENCE = (
    [0.62 + 0.02 * i for i in range(HALF_MAP_SIZE)]
    + [1.0]
    + [1.2 + 0.02 * i for i in range(HALF_MAP_SIZE)]
)
...
N_FLAG_TYPES = 5
I_SCORE_SURROUNDED_HALITE_GROUND = (
    0  # 隣接4マスのうち何マス halite 増産するか 途中更新なし shipyard 生成破壊で変わりうることに注意
)
...
def initialize_both_move_to_halite_nash_equilibrium_impl(halite):
    ...
    while 0.99 < abs(r_ok - r_ng):
        r_mid = 0.5 * (r_ng + r_ok)
        p, r_p = initialize_both_move_to_halite_nash_equilibrium_impl2(
            halite, r_mid * gamma
        )
...
class Project(object):
    ...
    def ships_generator(self, with_free=False, with_my_project=False):
        ...
        for ship in a.sorted_ships:
            ...
            if (with_free and (project_id is None)) or (
                with_my_project and (project_id == self.project_id)
            ):
                yield ship
    ...
    def discard(self):
        ...
        a.log(
            step=a.board.step,
            id_=None,
            s=f"p{a.player_id} discard project_id={self.project_id}",
        )
        ...
        assert (
            self.project_id not in a.belonging_project.values()
        ), f"project_id={self.project_id} belonging_project={a.belonging_project}"
    ...
    def join_project(self, *args, staff_ids, **kwargs):
        ...
        return self.agent.join_project(
            *args, staff_ids=staff_ids, project_id=self.project_id, **kwargs
        )
...
class DefenseShipyardProject(Project):
    ...
    def shipyard_defender_strategy(self, ship):
        ...
        a.log(
            step=a.board.step,
            id_=ship.id,
            s=f"yd{shipyard.id} shipyard_defender h{ship.halite} o_min_d={o_min_d} d={d} free_steps={free_steps}",
        )
        if free_steps < 0:  # こいつ任命できないはずなんだが
            a.log(
                loglevel="WARNING",
                id_=ship.id,
                s=f"defender too far. project_id={self.project_id} h={ship.halite} o_min_d={o_min_d} info={self.info}",
            )
        ...
        if o_min_d == 1:  # 身を盾にして守る
            for k_action, cell_k in enumerate(
                a.neighbor_cells(a.board.cells[ship.position])
            ):
        ...
        if free_steps == 0 or (
            player_halite < 1000 and short_halite <= ship.halite
        ):  # 最短距離で帰る
            a.log(
                id_=ship.id,
                s=f"{ship.position} prj={self.project_id} free_steps{free_steps} q_empty{q_empty} back short_halite{short_halite}",
            )
            return a.reserve_ship_by_q(ship, q=q_empty, priority=priority)
        # 今いる場所を掘れるかどうか検討しよう
        cell = a.board.cells[ship.position]
        mine_project = a.projects.get(
            f"mine_{ship.position[0]}_{ship.position[1]}", None
        )
        if mine_project:
        ...
        if free_steps == 1:  # 最短 / 1回停止可能
            if mine_threshold <= cell.halite:
                if 0 < ship.halite:  # 掘っても free_steps は減らない
                    a.log(
                        id_=ship.id,
                        s=f"{ship.position} prj={self.project_id} free_steps{free_steps} q_mine{q_mine} mine_thre{mine_threshold}",
                    )
                    ...
                    for cell_i in a.neighbor_cells(a.board.cells[shipyard.position]):
                        ...
                        if a.scores[I_SCORE_OPPONENT_REACH, i_, j_] < 1.5:
                            # blockできていないので大人しく帰る
                            return a.reserve_ship_by_q(
                                ship, q=q_empty, priority=priority
                            )
                    a.log(
                        id_=ship.id,
                        s=f"{ship.position} prj={self.project_id} blocking. q_mine={q_mine}",
                    )
                    return a.reserve_ship_by_q(ship, q=q_mine, priority=priority)
            a.log(
                id_=ship.id,
                s=f"{ship.position} prj={self.project_id} free_steps{free_steps} q_empty{q_empty} back",
            )
            return a.reserve_ship_by_q(ship, q=q_empty, priority=priority)
        ...
        if (
            ...
            for k_action, cell_k in enumerate(
                ...
                if d == 0:
                    if (
                        cell_k.ship
                        and cell_k.ship.player_id == a.player_id
                        and 0 < cell_k.ship.halite
                    ):
            ...
            a.log(
                id_=ship.id,
                s=f"{ship.position} prj={self.project_id} free_steps{free_steps} q_mine{q_mine} back with mine has_non_empty_ship={has_non_empty_ship} d{d} original_free_steps{original_free_steps} mine_thre{mine_threshold} mine_prj{mine_project}",
            )
            return a.reserve_ship_by_q(ship, q=q_mine, priority=priority)
        ...
        a.log(
            id_=ship.id,
            s=f"{ship.position} prj={self.project_id} free_steps{free_steps} q_explore{q_explore} max{max_position} h_diff{max_halite_diff}",
        )
    ...
    def schedule(self):
        ...
        if (
            len_ships == 1
            and a.board.current_player.ships[0].halite == 0
            and a.board.current_player.halite < MAX_HALITE
        ):
            a.log(
                loglevel="WARNING",
                id_=shipyard.id,
                s=f"prj={self.project_id} last_escape_mode",
            )
            return False  # 逃げ専
        ...
        if self.o is None:
            ...
            self.priority = (
                200000.0
                - o_min_d * 10000.0
                + a.scores[I_SCORE_HALITE_D4, self.i, self.j]
            )
    ...
    def should_cancel_impl(self, e, o, cell_o, e_min_d, o_min_d):
        ...
        condition0 = 3 == np.sum(
            (np.array(self.empty_o_min_d[-3:]) <= 1).astype(np.int32)
        )
        condition1 = 4 <= np.sum(
            (np.array(self.empty_o_min_d[-5:]) <= 1).astype(np.int32)
        )
        ...
        for k_action, cell_k in enumerate(
            a.neighbor_cells(a.previous_board.cells[self.position])
        ):
            if k_action == 0:
                continue
            ship_k = cell_k.ship
    ...
    def search_empty_ally(self):
        ...
        for i_action, cell_i in enumerate(
            self.agent.neighbor_cells(self.shipyard_cell)
        ):
            if i_action == 0:
                continue
            ship_i = cell_i.ship
    ...
    def run_cancel(self, e, target_position):
        ...
        if ally_ship:
            self.join_project(
                staff_ids=[ally_ship.id], role="defender_ally", forced=True
            )
            ...
            a.log(
                id_=e.id,
                s=f"prj={self.project_id} cancel to {target_position}, with ally s{ally_ship.id}",
            )
            ...
            if a.flags.get(I_FLAG_NEXT_SHIP_POSITION, i=self.i, j=self.j):
                a.log(
                    loglevel="warning",
                    s=f"prj={self.project_id} cannot spawn because someone will return type A",
                )
            ...
            elif self.spawn_step_threshold <= a.board.step and 1 < len(
                a.board.current_player.shipyards
            ):
                # もう SPAWN するのは勿体ないので, 抑止力で祈る
                self.reserve_budget(MAX_HALITE - self.budget)
                a.log(
                    id_=e.id,
                    s=f"prj={self.project_id} cancel to {target_position}, with fake SPAWN",
                )
                ...
                a.log(
                    id_=e.id,
                    s=f"prj={self.project_id} cancel to {target_position}, with SPAWN",
                )
    ...
    def run(self):
        ...
        if (
            a.board.step < self.spawn_step_threshold
            and 0 < self.budget
        ...
        for ship in self.defense_shipyard_strategy_ships_generator():
            ...
            if ship.halite == 0:
                ...
                if (d < n_min_d) or ((d == n_min_d) and (n.halite < ship.halite)):
                    n = ship
                    n_min_d = d
        if e:
        ...
        if n_min_d < o_min_d:  # deposit 間に合う
            ...
            target_position = self.should_cancel(
                e=e, o=o, cell_o=cell_o, e_min_d=e_min_d, o_min_d=o_min_d
            )
            if target_position:
                # RestrainShipyardProject 対策で相殺しに行く
                done, safe, spawned = self.run_cancel(
                    e=e, target_position=target_position
                )
            else:
                defender = e
                safe = True
            ...
            if someone_arrived:
                ...
                a.log(
                    loglevel="warning",
                    s=f"id={self.project_id} cannot spawn because someone will return type B",
                )
            elif (self.spawn_step_threshold_final <= a.board.step) or (
                self.spawn_step_threshold <= a.board.step
                and a.scores[I_SCORE_HALITE_D4, self.i, self.j] < 1000.0
                ...
                safe = False
                a.log(
                    f"prj={self.project_id} stop spawning because the shipyard is not tasty"
                )
            else:
                self.reserve_budget(-self.budget)
                a.reserve_shipyard(shipyard, ShipyardAction.SPAWN)
        ...
        if (
            ...
            if can_spawn:
                a.log(
                    f"prj={self.project_id} unneeded spawn freeh{a.free_halite} budget{self.budget}"
                )
                self.reserve_budget(-self.budget)
                a.reserve_shipyard(shipyard, ShipyardAction.SPAWN)
                spawned = True
        ...
        self.info["safe"] = safe
        a.log(
            s=f"id={self.project_id} run priority={self.priority} shipyard_info={self.info} ships={self.ships} spawned={spawned}"
        )
        return True
    ...
    def swap_project(self, ship):
        ...
        for k_action, cell_k in enumerate(
            ...
            if d < d_k:
                priority += d_k
                to_sort.append(
                    (priority, ship_k.id, ship_k, project_k, target_position)
                )
        if not to_sort:
            return ship
        # join, dismiss 処理をする
        priority, swapped_ship_id, swapped_ship, project, target_position = sorted(
            to_sort, reverse=True
        )[0]
        ...
        if project.project_id[:4] == "mine":
            ...
            a.log(
                id_=ship.id,
                s=f"swapped. old_escort_prj={old_escort_project_id}({len(old_escort_project.ships)} new_escort_prj={new_escort_project_id}({len(new_escort_project.ships)})",
            )
            a.log(
                id_=swapped_ship_id,
                s=f"swapped. old_escort_prj={old_escort_project_id}({len(old_escort_project.ships)} new_escort_prj={new_escort_project_id}({len(new_escort_project.ships)})",
        ...
        a.log(
            id_=ship.id,
            s=f"swapped. s{ship.id}(prj={a.belonging_project.get(ship.id, None)}) s{swapped_ship_id}({a.belonging_project.get(swapped_ship_id, None)}) dk{d_k} target{target_position}",
        )
        a.log(
            id_=swapped_ship_id,
            s=f"swapped. s{ship.id}(prj={a.belonging_project.get(ship.id, None)}) s{swapped_ship_id}({a.belonging_project.get(swapped_ship_id, None)}) dk{d_k} target{target_position}",
...
class RestrainShipyardProject(Project):
    ...
    def feedback(self, shipyard):
        ...
        if (
            2
            <= a.opponent_history[self.shipyard.player_id][
                "cancel_against_shipyard_attack"
            ][0]
        ):
    ...
    def schedule(self):
        ...
        for ally_shipyard_id_k, d_ally_shipyard_k in a.ally_shipyard_distances[
            self.shipyard_id
        ].items():
        ...
        self.dx = rotated_diff_position(
            self.ally_shipyard.position[0], self.shipyard.position[0]
        )
        self.dy = rotated_diff_position(
            self.ally_shipyard.position[1], self.shipyard.position[1]
        )
        if self.dx <= -2:
        ...
        a.log(
            s=f"prj={self.project_id} ships={list(self.ships.keys())} d_ally_shipyard={self.d_ally_shipyard} op_return_dir{self.opponent_return_directions}"
        )
        ...
        for x, y, d in neighbor_positions(d=2, p=self.shipyard.position):
            ...
            ship_i = cell_i.ship
            if ship_i and ship_i.player_id not in [
                a.player_id,
                self.shipyard.player_id,
            ]:
                # 他の敵が仕事している
        ...
        # self.priority = -1.0
            ...
            self.priority = len(self.zero_halite_positions) + 1.0
        if self.priority < 0.0:
            self.dismiss_project(staff_ids=list(self.ships.keys()))
        a.log(
            id_=self.shipyard_id,
            s=f"prj={self.project_id} prio{self.priority} has_opyd={has_neighbor_opponent_shipyard} already_in_pos={self.already_in_position} op_working={self.opponent_working}",
        )
        return True
    ...
    def calculate_target_position(self):
        ...
        for j in range(22):
            ...
            action = np.argmax(q)
            p = Point(
                x=mod_map_size_x(p[0] + DX[action]), y=mod_map_size_x(p[1] + DY[action])
            )
            positions.append(p)
    ...
    def run(self):
        ...
        a.log(
            s=f"prj={self.project_id} {target_position} len_ships{len_ships} len_restrain_ships{len_restrain_ships} len_staffs{len_staffs} len_restrain_ships_allowed{len_restrain_ships_allowed} len_diff{len_diff} max_len_ships{max_len_ships} restrain_ships{restrain_ships}"
        )
        ...
        for opponent_ship in a.board.players[self.shipyard.player_id].ships:
            if opponent_ship.halite == 0:
                continue
            dx_opponent = rotated_diff_position(
                opponent_ship.position[0], self.shipyard.position[0]
            )
            dy_opponent = rotated_diff_position(
                opponent_ship.position[1], self.shipyard.position[1]
            )
            ...
            if np.any(directions):
                opponent_ships.append(
                    (opponent_ship, dx_opponent, dy_opponent, directions)
                )
            a.log(
                s=f"prj={self.project_id}{self.shipyard.position} test_op{opponent_ship.id}{opponent_ship.position} dxo{dx_opponent} dyo{dy_opponent} dirs{directions}"
            )
        for ship in self.restrain_ships_generator():
            ...
            for opponent_ship, dx_opponent, dy_opponent, directions in opponent_ships:
                ...
                d_shipyard = a.opponent_shipyard_distances[opponent_ship.id][
                    self.shipyard_id
                ]
        ...
        opponent_ship_id = (
            None if target_opponent_ship is None else target_opponent_ship.id
        )
        a.log(
            id_=ship_id,
            s=f"prj={self.project_id} {target_position} score{score} d{d} dx{dx} dy{dy}, op{opponent_ship_id} next_dir{next_direction} op_return_dir{self.opponent_return_directions} {condition}",
        )
        self.dismiss_project(staff_ids=list(self.ships.keys()))
        role = "escape"
        if next_direction is None:  # target_position へ移動する
            mode = "escape"
            a.moving_ship_strategy(
                ship=ship, position=target_position, mode=mode, mine_threshold=None
            )
        else:
            mode = "cancel"
            position = calculate_next_position(ship.position, next_direction)
            a.moving_ship_strategy(
                ship=ship, position=position, mode=mode, mine_threshold=None
            )
...
class EscortProject(Project):
    ...
    def schedule(self):
        ...
        self.shipyard, self.d_target_staff = a.find_nearest_ally_shipyard(
        ...
        if self.shipyard:
            ...
            self.target_staff, self.d_target_staff = a.find_leader_ship(
                target_ship.position
            )
        ...
        if self.shipyard and (not is_project_mine):
            ...
            d_to_shipyard = calculate_distance(
                target_ship.position, self.shipyard.position
            )
            i_shipyard, j_shipyard = position_to_ij(self.shipyard.position)
            opponent_reach_shipyard = int(
                1e-6 + a.scores[I_SCORE_OPPONENT_REACH, i_shipyard, j_shipyard]
            )
            i_target, j_target = position_to_ij(target_ship.position)
            opponent_reach_target = int(
                1e-6 + a.scores[I_SCORE_OPPONENT_REACH, i_target, j_target]
            )
            if d_to_shipyard < opponent_reach_shipyard:
                ...
                d_to_shipyard <= 6
                and d_to_shipyard < opponent_reach_target + opponent_reach_shipyard
            ):
                # shipyard の反対側っぽいのでおおよそ安全
                a.log(
                    id_=self.target_ship_id,
                    s=f"{target_ship.position} EscortProject: target can ALMOST safely deposit to {self.shipyard.id}{self.shipyard.position}",
                )
                self.can_safely_deposit = True
        ...
        if defender_ship:  # 現職の検証
            ...
            if (
                ...
                or (
                    (defender_ship_project_id is not None)
    ...
    def run(self):
        ...
        is_defense_shipyard_project = (target_project_id is not None) and (
            target_project_id[:10] == "defense_yd"
        )
        if self.priority < 0.0 or is_defense_shipyard_project:
            self.dismiss_project(staff_ids=list(self.ships.keys()))
            return True
        if self.defender_ship_id and a.determined_ships.get(
            self.defender_ship_id, None
        ):
        ...
        if self.can_safely_deposit:  # defender 不要
            ...
            if self.defender_ship_id is not None:
                ...
                defender_ship_project_id = a.belonging_project.get(
                    self.defender_ship_id, None
                )
                if (
                    defender_ship_project_id is not None
        ...
        if determined is None:
            ...
            if self.is_final:
                if target_ship.halite == 0 and 5 <= len(a.board.current_player.ships):
                    ...
                    opponent_shipyard, d_opponent_shipyard = a.find_nearest_shipyard(
                        target_ship.position, player_ids=opponents
                    )
            ...
            if (
                398 <= a.board.step
                and 500 < target_ship.halite
                and 1 < self.d_target_staff
            ):
                a.reserve_ship(target_ship, ShipAction.CONVERT)
                a.log(
                    id_=target_ship.id, s=f"h{target_ship.halite} convert at last turn"
                )
            else:
                a.moving_ship_strategy(
                    target_ship,
                ...
                a.log(
                    id_=target_ship.id,
                    s=f"{target_ship.position}->{target_position} by myself in EscortProject is_final={self.is_final} mode={mode}",
                )
        else:
            target_position = None
        if defender:
            a.log(
                id_=defender.id,
                s=f"{defender.position} escort to {target_ship.id}{target_ship.position} is_final={self.is_final}",
            )
            a.log(
                id_=target_ship.id,
                s=f"{target_ship.position} escorted by {defender.id}{defender.position} is_final={self.is_final}",
            )
            ...
            if determined is None and 0 < target_ship.halite:
                ...
                mode = "cancel_without_shipyard"
            for i_action, cell_i in enumerate(
                a.neighbor_cells(a.board.cells[target_ship.position])
            ):
                ...
                q_i, forced = a.calculate_moving_ship_preference(
                    ship=defender,
                    position=cell_k.position,
                    mode=mode_i,
                    mine_threshold=None,
                )
                a.log(
                    id_=defender.id,
                    s=f"[{i_action}{cell_i.position}] ka{k_action}{cell_k.position} tdk{target_d_k} dk{d_k} mode={mode_i}",
                )
                qs[i_action, :] = q_i
            a.reserve_ship_by_q(ship=defender, q=qs, depend_on=target_ship)
        return True
...
class MineProject(Project):
    ...
    def __init__(self, position, *args, **kwargs):
        super().__init__(
            *args, project_id=f"mine_{position[0]}_{position[1]}", **kwargs
        )
    ...
    def run_miner(self, miner, forced=False):
        ...
        if determined is not None:
            ...
            if (not forced) or (determined != "reserved"):
                a.log(
                    loglevel="warning",
                    id_=miner.id,
                    s=f"prj={project_id} run_miner failed because determined{determined} or not forced",
                )
                exit()
                return
        ...
        if miner_d == 0:
            ...
            q, forced_ = a.calculate_moving_ship_preference(
                ...
                position=self.position,
                mode=mode,
                mine_threshold=self.mining_halite_threshold,
            )
            ...
            for k_action, cell_k in enumerate(a.neighbor_cells(target_cell)):
                ...
                if ship_k.halite < miner.halite:  # 無理
                    a.log(
                        s=f"prj={self.project_id} k{k_action} ship_k{ship_k.id} h{ship_k.halite}"
                    )
                    self.d_halite = 0.0
                    return True
                if ship_k.halite == miner.halite:
                    if k_action == 0:  # 先着されているので待って追い出したい
                        ...
                        if miner_cell.halite <= 3.99 or (
                            100.0 < miner_cell.halite < self.halite
                        ):  # 掘りあったら有利
                            wait_opponent_mining = True
                        a.log(
                            id_=miner.id,
                            loglevel="info",
                            s=f"prj={self.project_id} k{k_action} ship_k{ship_k.id} th{self.halite} miner_cellh{miner_cell.halite} op_mining={opponent_mining} wait_op_mining={wait_opponent_mining}",
                        )
            ...
            for player_id in range(PLAYERS):
                if not opponent_exists[player_id]:
                    continue
                t, u = a.opponent_history[ship_k.player_id].get(
                    "cancel_both_move_to_mine", [0, 0]
                )
                if u == 0:  # 初手は様子見
            ...
            if np.any(opponent_exists):
                a.log(
                    id_=miner.id,
                    s=f"prj={self.project_id} h{miner.halite} p_nash{p_nash:.6f} p_stay{p_stay} p_opponent_go{p_opponent_go:.6f} p_success{p_success:.6f} mode={mode} op_mining={opponent_mining} wait_op_mining={wait_opponent_mining}",
                )
            if wait_opponent_mining:  # 相殺覚悟で隣待ち
                mode = "cancel"
                q, forced_ = a.calculate_moving_ship_preference(
                    miner, position=miner.position, mode=mode, mine_threshold=3.99
                )
            else:
                q, forced_ = a.calculate_moving_ship_preference(
                    miner,
            ...
            q, forced_ = a.calculate_moving_ship_preference(
                ...
                position=self.position,
                mode=mode,
                mine_threshold=None if miner.halite == 0 else 300.0,
            )
        else:
            mode = "escape"
            q, forced_ = a.calculate_moving_ship_preference(
        ...
        a.reserve_ship_by_q(
            miner, q=q, forced=False, priority=priority, depend_on=depend_on
        )
        self.update_score()
        a.log(
            id_=miner.id,
            s=f"run_miner h{miner.halite} {miner.position}->{self.position} h{self.halite} prj={previous_project_id}->{self.project_id} prio{self.priority:.1f} noc{self.neighbor_empty_opponent_counter} h_thre{self.halite_threshold} mh_thre{self.mining_halite_threshold} mode={mode}",
        )
    ...
    def swap_mine_project(self, miner, defender, escort_project):
        ...
        for k_action, cell_k in enumerate(
            a.neighbor_cells(a.board.cells[miner.position])
        ):
            if k_action == 0:
                continue
            ship_k = cell_k.ship
            ...
            if project_id_k == f"escort{miner.id}":
                ...
                if (
                    project_k.defender_ship_id is not None
                    and project_k.defender_ship_id != defender_ship_id
                ):
        ...
        if not to_sort:
            a.log(
                id_=miner.id,
                s=f"prj={self.project_id} swap no_candidates. cond=({condition})",
            )
        ...
        if new_mine_project is None:  # defenderとの交換
            if defender and swapped_ship_id == defender_ship_id:
                ...
                a.log(
                    id_=miner.id,
                    s=f"prj={self.project_id} swap_failed. cond=({condition})",
                )
            return miner, False, self, escort_project
        # miner 同士の交換
        ...
        a.log(
            id_=miner.id,
            s=f"prj={self.project_id} swapped. s{miner.id}(prj={a.belonging_project.get(miner.id, None)}) s{swapped_ship_id}({a.belonging_project.get(swapped_ship_id, None)}) d0{d0} dk{d_k} target{target_position} cond({condition})",
        )
        a.log(
            id_=swapped_ship_id,
            s=f"prj={self.project_id} swapped. s{miner.id}(prj={a.belonging_project.get(miner.id, None)}) s{swapped_ship_id}({a.belonging_project.get(swapped_ship_id, None)}) d0{d0} dk{d_k} target{target_position} cond({condition})",
    ...
    def mine_project_ships_generator(self, max_d):
        ...
        for ship in a.sorted_ships:
            ...
            if self.shipyard:
                ...
                if (
                    nearest_shipyard is not None
                    and nearest_shipyard.id != self.shipyard.id
                ):
            ...
            if project is None:
                ...
                if project.d_halite + threshold < self.d_halite:
                    a.log(
                        f"prj={self.project_id} other_prj={project_id} d{d} d_other{d_other} dh{self.d_halite} dh_other{project.d_halite}"
                    )
                    yield ship, d
    ...
    def update_halite_threshold(self):
        ...
        if ally_d <= opponent_d and danger_zone < 0.5:
            ...
            if (
                3 <= len_ships
                and 0.5 < hunt_zone
                and self.last_mined_player_id == a.player_id
            ):
                if step < 20:
        ...
        condition += (
            f" h_thre{self.halite_threshold} mh_thre{self.mining_halite_threshold}"
        )
        return condition
    ...
    def schedule(self):
        ...
        if self.shipyard is None:
            ...
            if 389 < a.board.step:
                ...
                len_ships <= 1 and a.board.current_player.halite < 2 * MAX_HALITE
            ):  # もう拠点を構えるのは不可能
        ...
        self.empty_ally_reach = int(
            1e-6 + a.scores[I_SCORE_EMPTY_ALLY_REACH, self.i, self.j]
        )
        self.ally_reach_v2 = min(self.ally_reach + 1, self.empty_ally_reach)
        self.opponent_reach = int(
            1e-6 + a.scores[I_SCORE_OPPONENT_REACH, self.i, self.j]
        )
        self.empty_opponent_reach = int(
            1e-6 + a.scores[I_SCORE_EMPTY_OPPONENT_REACH, self.i, self.j]
        )
        ...
        if self.priority < 0.0:
            ...
            self.reset_project()
        a.log(
            f"prj={self.project_id} schedule prio{self.priority:.1f} cond=({condition}) h_thre{self.halite_threshold}"
        )
        return True
    ...
    def can_offer(self, ship_halite, d):
        ...
        if ship_halite == 0:
            ...
            self.d_halite, condition = self.calculate_minable_halite(
                advantage=advantage
            )
            # あんまり遠いならjoinさせない
            return d <= 8 and 4.9 < self.d_halite
        if 1000.0 < ship_halite and 0 < d:
            return False
        ...
        if d == 0:  # もうちょっと詳しくチェックする
            opponent_halite = int(
                1e-6 + a.scores[I_SCORE_MIN_NEIGHBOR_OPPONENT_HALITE, self.i, self.j]
            )
    ...
    def is_current_ship_mining(self):
        ...
        if cell.ship and cell.ship.player_id == a.player_id:
            ...
            a.log(
                s=f"prj={self.project_id} is_current_ship_mining s{cell.ship.id} determined={determined}"
            )
            if determined is None:
                pass
            elif determined == "reserved":
    ...
    def should_have_defender(self, d_shipyard):
        ...
        a.log(
            f"prj={self.project_id} should_have_defender d_yd{d_shipyard} o1{o1} d_thre{d_threshold[o1]}"
        )
    ...
    def run(self):
        ...
        for i, (
            ship,
            d,
            negative_escorted_count,
            negative_halite,
            ship_id,
        ) in enumerate(sorted(to_sort, key=itemgetter(1, 2, 3, 4))):
        ...
        self.join_project(staff_ids=[miner.id], role="miner")
        old_miner = miner
        miner, reserved, old_miner_project, escort_project = self.swap_mine_project(
            miner, defender, escort_project
        )
        if old_miner_project.project_id == self.project_id:
            # かわりませんでした
            self.run_miner(miner)
        elif (
            escort_project and old_miner_project.project_id == escort_project.project_id
        ):
            # 護衛と交代しました
            defender = old_miner
            self.run_miner(miner)
        ...
        if escort_project:  # old_miner に対する defender も一緒に動かしてしまう
            ...
            if defender:
                a.log(
                    id_=defender_ship_id,
                    s=f"{defender.position} hired by prj={self.project_id}, old_miner{old_miner.id}{old_miner.position} escort_project{escort_project.project_id} prio{escort_project.priority} defprj={a.belonging_project.get(defender_ship_id, None)}",
                )
            a.log(
                id_=old_miner.id,
                s=f"{old_miner.position} hire defender{defender_ship_id} to escort prio{escort_project.priority} defprj={a.belonging_project.get(defender_ship_id, None)}",
            )
...
class ExpeditionProject(Project):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.shipyard_halite_threshold = 1500.0
    ...
    def schedule(self):
        ...
        max_len_opponent_ships = np.max(
            [len(a.board.players[player_id].ships) for player_id in range(PLAYERS)]
        )
        if self.last_converted_position:
            ...
            if shipyard and shipyard.player_id == a.player_id:
                ...
                if project:
                    ...
                    a.log(
                        id_=shipyard.id,
                        s=f"hand over budget {self.project_id}={budget} -> {project_id}={project.budget}",
                    )
                else:
                    a.log(
                        loglevel="warning",
                        s=f"prj={self.project_id} hand over failed. {project_id} not found yd{shipyard.id}{shipyard.position}",
                    )
                    pass
            # 一旦解散
            return False
        ...
        if a.board.step < a.last_shipyard_attacked_step + 50:
            ...
            (2 <= a.board.step < 200)
            and (len_shipyards <= 2)
            and (len_shipyards * 7 <= len_ships)
        ):
            self.priority = 1e8
        elif (
            (self.early_game_threshold <= a.board.step)
            and (len_shipyards * 6 <= len_ships)
            and (25000.0 < a.world_halite or 300.0 < a.halite_per_ship)
        ):
            # 敵が狩りタイプばっかりだと中盤起こる
            self.priority = 1e8
        elif (
            len_shipyards <= 10
            and len_shipyards * 4 < len_ships
            and max_len_opponent_ships <= len_ships
        ):
            self.priority = 1e6
        elif self.ships:
            self.priority = 1e6
    ...
    def search_target_position(self):
        ...
        while not queue.empty():
            ...
            score = self.calculate_shipyard_position_score(
                position, min_best_d, d, is_empty_ship
            )
            if best_score < score:
                best_score = score
                ...
                if min_best_d is None:
                    min_best_d = d
                a.log(
                    f"prj={self.project_id} score_updated. d{best_d} score{score:.1f} {position} s{best_ship.id} h{best_ship.halite}"
                )
        ...
        a.log(
            s=f"search_target_position={best_position}, leader={best_ship_id} d={best_d} score={best_score}"
        )
    ...
    def calculate_shipyard_position_score(self, position, min_best_d, d, is_empty_ship):
        ...
        score_opponent_shipyard_d4 = 1 / max(
            1.0, a.scores[I_SCORE_OPPONENT_SHIPYARD_D4, i, j]
        )
        score = (
            score_candidates
            * score_hunt_zone
            * score_gamma
            * score_opponent_shipyard_d4
        )
    ...
    def leader_ship_strategy(self, ship, d):
        ...
        if d == 0:  # convert予定地
            ...
            for ship_id_c in self.ships.keys():
                ...
                if (d_c < opponent_reach) or (
                    ...
                    break
            sufficient_budget = (
                2 - safely_convert_without_spawn
            ) * MAX_HALITE <= self.budget + ship.halite
            if (
                a.board.cells[ship.position].halite < self.mine_threshold_before_convert
                ...
                a.moving_ship_strategy(
                    ship, position=self.target_position, mode="mine", mine_threshold=4.0
                )
                condition = "target_reached_mine"
        else:
            a.moving_ship_strategy(
                ship, position=self.target_position, mode="escape", mine_threshold=None
            )
        ...
        a.log(
            id_=ship.id,
            s=f"prj={self.project_id} leader {self.target_position} a{i_action} cond({condition})",
        )
    ...
    def coworker_ship_strategy(self, ship, d, leader_ship):
        ...
        to_sort = []
        for k_action, cell_k in enumerate(
            a.neighbor_cells(a.board.cells[self.target_position])
        ):
            ...
            if cell_k.halite < 1e-6:
                ...
                mine_project = a.projects.get(
                    f"mine_{cell_k.position[0]}_{cell_k.position[1]}", None
                )
                if mine_project:
            ...
            if k_action == 0:
                ...
                if self.last_converted_position:  # leaderはこのstepでconvertしてくる
                    if 0 < ship.halite:
                        ...
                        opponent_reach = int(
                            1e-6 + a.scores[I_SCORE_OPPONENT_REACH, i_, j_]
                        )
                        if d == 1 and opponent_reach <= 1:
                            priority = 10001.0
                        elif d <= opponent_reach and (
                            self.budget + leader_ship.halite < 2 * MAX_HALITE
                        ):
                            priority = 10002.0
            a.log(
                id_=ship.id,
                s=f"prj={self.project_id} coworker k{k_action}{cell_k.position} prio{priority} h{cell_k.halite} halite_k{halite_k} steps_k{steps_k} budget{self.budget}",
            )
        ...
        if not to_sort:
            ...
            priority, k_action, mine_threshold_k, cell_k = sorted(
                to_sort, key=itemgetter(0, 1), reverse=True
            )[0]
        ...
        a.log(
            id_=ship.id,
            s=f"prj={self.project_id} coworker {self.target_position}{k_action}->{target_position} prio{priority}",
        )
    ...
    def run(self):
        ...
        a.log(
            s=f"prj={self.project_id} target_position={self.target_position} ships={list(self.ships.keys())}+{staff_ids}"
        )
...
class HuntProject(Project):
    ...
    def schedule(self):
        ...
        if self.opponent_safe_direction < 0:
            ...
            for x_k, y_k, d_k in neighbor_positions(d=2, p=self.target_ship.position):
                ...
                if (
                    (ship_k is None)
                    or (ship_k.player_id != self.target_ship.player_id)
                    or (0 < ship_k.halite)
                ):
                    continue
                defender_count += 1
        ...
        for shipyard in a.board.players[self.target_ship.player_id].shipyards:
            dx = rotated_diff_position(
                self.target_ship.position[0], shipyard.position[0]
            )
            dy = rotated_diff_position(
                self.target_ship.position[1], shipyard.position[1]
            )
            abs_dx = abs(dx)
            abs_dy = abs(dy)
            if abs_dx <= abs_dy:
    ...
    def assign_candidates(self):
        ...
        north_position = Point(x=p0[0], y=mod_map_size_x(p0[1] + self.dy_limit[1]))
        east_position = Point(x=mod_map_size_x(p0[0] + self.dx_limit[1]), y=p0[1])
        south_position = Point(x=p0[0], y=mod_map_size_x(p0[1] + self.dy_limit[0]))
        west_position = Point(x=mod_map_size_x(p0[0] + self.dx_limit[0]), y=p0[1])
        a.log(
            id_=self.target_ship_id,
            s=f"{p0} n{north_position} e{east_position} s{south_position} w{west_position} dx_limit{self.dx_limit} dy_limit{self.dy_limit}",
        )
        for ship in a.sorted_ships:
        ...
        for direction, ships in enumerate(candidates):
            ...
            if strategy == 5:  # 双方向
                if len(ships) <= 1:
                    a.log(
                        loglevel="warning", s=f"strategy == 5. len(ships)={len(ships)}"
                    )
        ...
        for direction in range(1, LEN_MOVE):
            ...
            if 0 == len_c:
                ...
                a.log(
                    logloevel="warning",
                    s=f"direction={direction} ship candidate not found target{self.target_ship.position} c1={c1} c2={c2} diag_distribution={diag_distribution} dp_index={dp_index}",
                )
                continue
            for j in range(2):
                if len_c <= j:
                ...
                self.join_project(
                    staff_ids=[ship_j.id], role=str(direction), forced=True
                )
        ...
        a.log(
            id_=self.target_ship_id,
            s=f"assign_candidates 4 target{self.target_ship.id}{self.target_ship.position} ships{ships_to_log()} center{self.center_direction}",
        )
        return True
    ...
    def run(self):
        ...
        for ship, d, ship_id in self.sorted_ships:
            ...
            if role == I_NORTH:
                if ship.position[0] == p1[0]:  # x座標はあっている
                    if (
                        ship.position[1] != p1[1]
                        or 0 == get_halite
                        or (0 < ship.halite and (not last_stop))
                    ):
                        pass  # 普通に目標地点向かって距離詰めたり停止すればOK
                    elif (opponent_safe_direction & mask_ew) == (
                        1 << I_EAST
                    ):  # 東は安全だから東行くっしょ
                ...
                if ship.position[1] == p1[1]:  # y座標はあっている
                    if (
                        ship.position[0] != p1[0]
                        or 0 == get_halite
                        or (0 < ship.halite and (not last_stop))
                    ):
                        pass  # 普通に目標地点向かって距離詰めたり停止すればOK
                    elif (opponent_safe_direction & mask_ns) == (
                        1 << I_NORTH
                    ):  # 北は安全だから北行くっしょ
                        p2 = cell_r.north.position
                    elif (opponent_safe_direction & mask_ns) == (
                ...
                if ship.position[1] == p1[1]:  # y座標はあっている
                    if (
                        ...
                        pass  # 普通に目標地点向かって距離詰めたり停止すればOK
                    if (opponent_safe_direction & mask_ns) == (
                        1 << I_NORTH
                    ):  # 北は安全だから北行くっしょ
                        p2 = cell_r.north.position
                    elif (opponent_safe_direction & mask_ns) == (
        ...
        for ship, p1, p2, d_r, last_stop, should_challenge in positions:
            ...
            a.log(
                id_=self.target_ship_id,
                s=f"p0{p0} s{ship.id}{ship.position}->p1{p1}/p2{p2} checkmate{checkmate_count} p{p} role{self.ships[ship.id]} center{self.center_direction} center_ship{center_ship_id} should_challenge={should_challenge} safe_dir{opponent_safe_direction}",
            )
            a.moving_ship_strategy(
                ship,
                position=p,
...
class MyAgent(object):
    def __init__(self, player_id, *args, verbose, **kwargs):
        ...
        self.belonging_project = (
            {}
        )  # key is ship_id or shipyard_id, value is project_id
    ...
    def join_project(self, project_id, staff_ids, role="no_role", forced=False):
        ...
        for staff_id in staff_ids:
            ...
            if (previous_project_id is not None) and (
                project_id != previous_project_id
            ):
                if not forced:
                    self.log(
                        loglevel="warning",
                        s=f"p{self.player_id} staff_id={staff_id} join to {project_id} but it already belongs to {self.belonging_project[staff_id]}",
                    )
    ...
    def log(self, s, step=None, id_=None, indent=0, loglevel="DEBUG"):
        ...
        if ((self.verbose & 4) == 4) or (
            (self.verbose & 1) == 1 and logging.DEBUG < level
        ):
    ...
    def update_opponent_history(self):
        if self.previous_board is None:
            self.log(
                step=self.board.step,
                id_=None,
                s=f"update_opponent_history: previous_board is None",
            )
            return  # 初手は処理しない
        ...
        for previous_shipyard in self.previous_board.shipyards.values():
            ...
            depositor_result = (
                None  # 0: depositしなかった, 1: deposit強行した, None: 無効かdepositorいない
            )
            ...
            for previous_cell_i in self.neighbor_cells(previous_cell):
                ...
                if ship is None:
                    ...
                    if ship.halite == 0:
                        ...
                        if current_ship:
                            attacker_result = (
                                1 if (current_ship.position == ship.position) else 0
                            )
                            self.opponent_history[ship.player_id][
                                "stop_shipyard_neighbor"
                            ][0] += attacker_result
                            self.opponent_history[ship.player_id][
                                "stop_shipyard_neighbor"
            ...
            if (len(defenders) == 0) and self.previous_board.players[
                player_id
            ].halite < MAX_HALITE:
            ...
            if result is not None:
                # 分子: 守った回数
                self.opponent_history[player_id]["defense_against_shipyard_attack"][
                    0
                ] += result
                # 分母: 試行回数
                self.opponent_history[player_id]["defense_against_shipyard_attack"][
            ...
            if depositor_result is not None:
                self.opponent_history[player_id]["deposit_against_shipyard_attack"][
                    0
                ] += depositor_result
                self.opponent_history[player_id]["deposit_against_shipyard_attack"][
                    1
            ...
            if cancel_result is not None:
                self.opponent_history[player_id]["cancel_against_shipyard_attack"][
                    0
                ] += cancel_result
                self.opponent_history[player_id]["cancel_against_shipyard_attack"][
                    1
            ...
            if shipyard_attacked_result is not None:
                self.opponent_history[player_id]["shipyard_attacked"][
                    0
                ] += shipyard_attacked_result
        ...
        for ship_id, previous_ship in self.previous_board.ships.items():
            ...
            if ship is None:
                self.log(
                    id_=ship_id,
                    s=f"{previous_ship.position} h{previous_ship.halite} p{player_id} dead",
                )
                pass
            ...
            mine_here = False
            i_action = None
            i_action_candidates = np.zeros(
                LEN_MOVE, dtype=np.bool
            )  # ship いなくなってしまった時に相殺対象がいうる方向
            for k_action, cell_k0 in enumerate(
                self.neighbor_cells(self.previous_board.cells[previous_ship.position])
            ):
                ground_halite[k_action] = cell_k0.halite
                for l_action, cell_l0 in enumerate(
                    self.neighbor_cells(self.previous_board.cells[cell_k0.position])
                ):
                    if not cell_l0.ship:
                        continue
                ...
                if k_action == 0:
                    ...
                    cell_k0.ship
                    and cell_k0.ship.player_id != player_id
                    and cell_k0.ship.halite == 0
                ):
                    empty_opponent[k_action] = True
            ...
            if np.any(has_ground_halite):
                ...
                if np.any(t):
                    self.opponent_history[player_id]["cancel_move_to_mining_opponent"][
                        1
                    ] += 1
                    if np.any(np.logical_and(t, i_action_candidates)):
                        self.opponent_history[player_id][
                            "cancel_move_to_mining_opponent"
                        ][0] += 1
                ...
                if np.any(t):
                    ...
                    if np.any(np.logical_and(t, i_action_candidates)):
                        self.opponent_history[player_id]["cancel_both_move_to_mine"][
                            0
                        ] += 1
    ...
    def update_projects(self):
        ...
        projects = sorted(
            list(self.projects.values()), key=project_key_fn, reverse=True
        )
        for project in projects:
            ...
            if self.free_halite < 0 or project.budget < 0:  # バグっていそう
                self.log(
                    loglevel="warning",
                    s=f"free_halite < 0 project_id={project.project_id}, project.budget={project.budget} halite={self.board.current_player.halite} free_halite={self.free_halite} discard",
                )
                project.discard()
        ...
        for shipyard_id, shipyard in self.board.shipyards.items():
            ...
            if shipyard.player_id == self.player_id:
                project = DefenseShipyardProject(agent=self, shipyard_id=shipyard_id)
            else:
                continue  # 無いほうが相殺減って強そう
                project = RestrainShipyardProject(agent=self, shipyard_id=shipyard_id)
        ...
        for shipyard_id_k, shipyard_k in shipyards.items():
            ...
            for shipyard_id_l, shipyard_l in shipyards.items():
                ...
                if self.has_internal(
                    i_k, j_k, i_l, j_l, di_kl, dj_kl, shipyard_k.player_id
                ):
                    continue
            ...
            for j_k in range(COLS):
                ...
                for shipyard_l in my_shipyards:
                    ...
                    if abs_di_kl + abs_dj_kl <= 3:
                        ...
                        break
                    if self.has_internal(
                        i_k, j_k, i_l, j_l, di_kl, dj_kl, self.player_id
                    ):
                        continue
                    ranges_in_i, ranges_out_i = self.split_ranges(i_k, i_l, di_kl)
                    ranges_in_j, ranges_out_j = self.split_ranges(j_k, j_l, dj_kl)
                ...
                if not has_near_shipyard:
                    self.scores[I_SCORE_FUTURE_HUNT_ZONE, i_k, j_k] = np.sum(
                        np.logical_and(visited, valid)
                    )
    ...
    def update_score_detour_distance(self):
        ...
        while queue:
            ...
            if self.scores[I_SCORE_DETOUR_REACH, i_k, j_k] < 99999.0:
                continue
            ...
            for i_l, j_l, d_l in neighbor_positions(d=1, p=(i_k, j_k)):
                if d_l == 0:
                    continue
                if self.scores[I_SCORE_DETOUR_REACH, i_l, j_l] < 99990.0:
                    continue
                new_d = d_k
        ...
        while opponent_queue:
            ...
            if self.scores[I_SCORE_OPPONENT_DETOUR_REACH, i_k, j_k] < 99999.0:
                continue
            shipyard_k = shipyards[shipyard_id_k]
            ...
            for i_l, j_l, d_l in neighbor_positions(d=1, p=(i_k, j_k)):
                if d_l == 0:
                    continue
                if self.scores[I_SCORE_OPPONENT_DETOUR_REACH, i_l, j_l] < 99990.0:
                    continue
        ...
        self.scores[I_SCORE_DETOUR_ADVANTAGE, ...] = (
            self.scores[I_SCORE_OPPONENT_DETOUR_REACH, ...]
            - self.scores[I_SCORE_DETOUR_REACH, ...]
        )
        ...
        self.ships_by_shipyard = {
            shipyard.id: {} for shipyard in self.board.current_player.shipyards
        }
    ...
    def update_score(self):
        ...
        for di, dj, d in neighbor_d_positions(d=1):
            ...
            self.scores[I_SCORE_SURROUNDED_HALITE_GROUND] += np.roll(
                np.roll(b, shift=di, axis=0), shift=dj, axis=1
            )
        ...
        for di, dj, d in neighbor_d_positions(d=3):
            ...
            self.scores[I_SCORE_HALITE_D4] += np.roll(
                np.roll(self.scores[I_SCORE_HALITE], shift=di, axis=0), shift=dj, axis=1
            )
            if d == 0:
        ...
        for ship_id, ship in self.board.ships.items():
            i, j = position_to_ij(ship.position)
            if ship.player_id == self.player_id:
                self.update_score_impl(
                    I_SCORE_ALLY_D2, p=ship.position, d=2, max_score=1.0
                )
                ...
                if ship.halite == 0:
                    ...
                    self.update_score_impl(
                        I_SCORE_NON_EMPTY_ALLY_D4, p=ship.position, d=4
                    )
            else:
                self.update_score_impl(
                    I_SCORE_OPPONENT_D2, p=ship.position, d=2, max_score=1.0
                )
                if ship.halite == 0:
                    self.update_score_impl(
                        I_SCORE_EMPTY_OPPONENT_D2, p=ship.position, d=2, max_score=1.0
                    )
                else:
                    self.update_score_impl(
                        I_SCORE_NON_EMPTY_OPPONENT_D2,
                        p=ship.position,
                        d=2,
                        max_score=1.0,
                    )
                self.update_score_impl(
                    I_SCORE_OPPONENT_D3, p=ship.position, d=3, max_score=1.0
                )
                for i2, j2, d2 in NEIGHBOR_POSITIONS[1][i, j]:
                    self.scores[I_SCORE_MIN_NEIGHBOR_OPPONENT_HALITE, i2, j2] = min(
                        ship.halite,
                        self.scores[I_SCORE_MIN_NEIGHBOR_OPPONENT_HALITE, i2, j2],
                    )
        ...
        for ship_id, shipyard in self.board.shipyards.items():
            if shipyard.player_id == self.player_id:
                self.update_score_impl(
                    I_SCORE_ALLY_SHIPYARD_D1, p=shipyard.position, d=1
                )
                self.update_score_impl(
                    I_SCORE_ALLY_SHIPYARD_D4, p=shipyard.position, d=4
                )
                self.update_score_impl(
                    I_SCORE_ALLY_SHIPYARD_D7, p=shipyard.position, d=7
                )
            else:
                self.update_score_impl(
                    I_SCORE_OPPONENT_SHIPYARD_D6, p=shipyard.position, d=6
                )
                self.update_score_impl(
                    I_SCORE_OPPONENT_SHIPYARD_D2, p=shipyard.position, d=2
                )
                self.update_score_impl(
                    I_SCORE_OPPONENT_SHIPYARD_D3, p=shipyard.position, d=3
                )
                self.update_score_impl(
                    I_SCORE_OPPONENT_SHIPYARD_D4, p=shipyard.position, d=4
                )
        ...
        self.ally_ship_distances = (
            {}
        )  # dict dict, first key: 全staff_id, second key: 味方ship_idのみ
        self.ally_shipyard_distances = (
            {}
        )  # dict dict, first key: 全staff_id, second key: 味方shipyard_idのみ
        self.opponent_ship_distances = (
            {}
        )  # dict dict, first key: 全staff_id, second key: 敵ship_idのみ
        self.opponent_shipyard_distances = (
            {}
        )  # dict dict, first key: 全staff_id, second key: 敵shipyard_idのみ
        ...
        self.update_score_impl_pq(
            I_SCORE_ALLY_REACH,
            initial_ship_positions=self.ally_ship_positions,
            initial_shipyard_positions=self.ally_shipyard_positions,
            obstacle_score_type=I_SCORE_OPPONENT_SHIPYARD_D2,
        )
        self.update_score_impl_pq(
            I_SCORE_EMPTY_ALLY_REACH,
            initial_ship_positions=self.empty_ally_ship_positions,
            initial_shipyard_positions=self.ally_shipyard_positions,
            obstacle_score_type=I_SCORE_OPPONENT_SHIPYARD_D2,
        )
        self.update_score_impl_pq(
            I_SCORE_NON_EMPTY_ALLY_REACH,
            initial_ship_positions=self.non_empty_ally_ship_positions,
            initial_shipyard_positions=[],
            obstacle_score_type=I_SCORE_OPPONENT_SHIPYARD_D2,
        )
        self.update_score_impl_pq(
            I_SCORE_OPPONENT_REACH,
            initial_ship_positions=self.opponent_ship_positions,
            initial_shipyard_positions=self.opponent_shipyard_positions,
            obstacle_score_type=I_SCORE_ALLY_SHIPYARD_D1,
        )
        self.update_score_impl_pq(
            I_SCORE_EMPTY_OPPONENT_REACH,
            initial_ship_positions=self.empty_opponent_ship_positions,
            initial_shipyard_positions=self.opponent_shipyard_positions,
            obstacle_score_type=I_SCORE_ALLY_SHIPYARD_D1,
        )
        self.update_score_impl_pq(
            I_SCORE_NON_EMPTY_OPPONENT_REACH,
            initial_ship_positions=self.non_empty_opponent_ship_positions,
            initial_shipyard_positions=[],
            obstacle_score_type=I_SCORE_ALLY_SHIPYARD_D1,
        )
        self.scores[I_SCORE_REACH_ADVANTAGE, ...] = (
            self.scores[I_SCORE_OPPONENT_REACH] - self.scores[I_SCORE_ALLY_REACH]
        )
    ...
    def store_distance_into_dict(self, ha, ho, staff_is, staff_js):
        for staff_i in staff_is:
            ha_i, ho_i = self.store_distance_into_dict_impl(
                staff_i=staff_i, staff_js=staff_js
            )
            ha[staff_i.id] = ha_i
            ho[staff_i.id] = ho_i
    ...
    def can_spawn(self, shipyard, budget=0, len_ships=None):
        ...
        if budget_level < 1:
            self.log(
                id_=shipyard.id,
                s=f"cannot spawn: budget_level{budget_level} freeh{self.free_halite} budget{budget}",
            )
            return False
        ...
        if self.board.step < self.greedily_spawn_step_threshold:
            self.log(
                id_=shipyard.id,
                s=f"can spawn: greedily_spawn_step_threshold{self.greedily_spawn_step_threshold}",
            )
            return True
        ...
        if remaining_steps < 50:
            self.log(
                id_=shipyard.id, s=f"cannot spawn: remaining_steps{remaining_steps}"
            )
            return False
        halite_per_ship_threshold = 1000.0
        if remaining_steps < 100:
            halite_per_ship_threshold = [180.0, 220.0, 300.0]
        elif remaining_steps < 150:
            halite_per_ship_threshold = [150.0, 180.0, 250.0]
        elif remaining_steps < 200:
            halite_per_ship_threshold = [120.0, 150.0, 220.0]
        else:
            halite_per_ship_threshold = [90.0, 120.0, 150.0]
        ...
        self.log(
            id_=shipyard.id,
            s=f'can{"" if ok else "not"} spawn: budget_level{budget_level} budget_threshold{budget_threshold} {condition}',
        )
        return ok
    ...
    def convert_strategy(self):
        ...
        my_shipyards = self.board.players[
            self.player_id
        ].shipyards  # player経由だとdictでなくlist
        ...
        for ship in self.sorted_ships:
            ...
            if ship.halite < halite_threshold:
                continue  # halite 不足
            for i_action, cell in enumerate(
                self.neighbor_cells(self.board.cells[ship.position])
            ):
                if cell.shipyard:
                ...
                if i_action != 0 and self.flags.get(
                    I_FLAG_NEXT_SHIP_POSITION, i=i, j=j
                ):
                    continue  # ぶつかる
                ...
                if (0 < len_my_shipyards) and (surrounding_halite < 1500.0):
                    continue  # 周囲がおいしくない
    ...
    def reserve_ship(self, ship, next_action, forced=False):
        ...
        if (
            (ship is None)
            or (ship.player_id != self.player_id)
        ...
        if next_action == ShipAction.CONVERT:
            ...
            if self.free_halite < 0:
                self.log(
                    loglevel="warning",
                    s=f"s{ship.id} attempt to convert without free_halite={self.free_halite}",
                )
            # scores を更新
            ...
            m = self.flags.get(
                I_FLAG_NEXT_SHIP_POSITION, x=next_position[0], y=next_position[1]
            )
            if m and (not forced):
                self.log(
                    loglevel="warning",
                    s=f"s{ship.id} attempt to set reserved position. position={ship.position}, next_action={next_action}, next_position={next_position}, m={m}",
                )
            else:
                self.log(
                    id_=ship.id,
                    s=f"reserve_ship {ship.position}->{next_position} next_action={next_action} m{m}",
                )
    ...
    def moving_ship_strategy(self, ship, position, mode, mine_threshold=None, q=None):
        q, forced = self.calculate_moving_ship_preference(
            ship=ship, position=position, mode=mode, mine_threshold=mine_threshold, q=q
        )
    ...
    def scheduled_next_action(self, ship_id):
        ...
        if not determined:
            ...
            if not reserving:
                self.log(
                    loglevel="warning",
                    s=f"s{ship_id} determined==reserve but reserving not found",
                )
                return None
            ...
            if len(q.shape) == 2:
                ...
                i_next_action2 = None
            assert (
                0 <= i_next_action < LEN_MOVE
            ), f"i_next_action{i_next_action} q{q} i_next_action2={i_next_action2}"
            return MOVE[i_next_action]
        return determined[0]

    def calculate_moving_ship_preference(
        self, ship, position, mode, mine_threshold=None, q=None
    ):
        """
        mode
            raw: 敵味方無視してシンプルに最速
        ...
        if mine_threshold is None and mode == "mine":
            self.log(
                loglevel="warning",
                s=f'moving_ship_strategy: mine_threshold is None and mode == "mine"',
            )
        ...
        for i_action, cell in enumerate(
            ...
            opponent_halite = int(
                1e-6 + self.scores[I_SCORE_MIN_NEIGHBOR_OPPONENT_HALITE, i2, j2]
            )

            ally_mining = False
            if (
                cell.ship
                and cell.ship.id != ship.id
                and 3.99 < cell.halite
                ...
                if next_action is None:  # 掘る / 不明
                    q[i_action] *= 0.9
                    condition += (
                        f" cellh{cell.halite}_but_ally_mining mine_thre{mine_threshold}"
                    )
                    ally_mining = True
                ...
                and (  # emptyから掘りたい時は相殺もやむなし
                    not (
                        (
                            ship.halite == 0
                            and mode == "mine"
                            and mine_threshold <= cell.halite
                            and i_action == 0
                        )
        ...
        self.log(
            id_=ship.id,
            s=f"mv prj={self.belonging_project.get(ship.id, None)} cond({condition})",
        )
        if mode == "merge":
            i_action = np.argmax(q)
            return q, True
        ...
        self.log(
            id_=ship.id,
            s=f"mv {ship.position}->{position} mode={mode} mine_th={mine_threshold} q={q}",
        )

        if ship.position[0] == position[0] and ship.position[1] == position[1]:
            # 周囲のpreferenceは帰還を想定しておく
            q1, nearest_shipyard, nearest_shipyard_distance = self.returning_preference(
                ship
            )
            ...
            self.log(
                id_=ship.id,
                s=f"moving_ship_strategy target reached. q1={q1} q2={q2} q={q}",
            )
        return q, False
    ...
    def mining_ship_strategy(self, ship, q=None, escape_mode=False, shipyard=None):
        ...
        (
            preference,
            nearest_shipyard,
            nearest_shipyard_distance,
        ) = self.returning_preference(ship, shipyard=shipyard)
        if q is None:
            q = np.ones(LEN_MOVE, dtype=np.float32)
        q *= preference
        q_mine = np.ones(LEN_MOVE, dtype=np.float32)
        ...
        self.log(
            step=self.board.step,
            id_=ship.id,
            s=f"h{ship.halite} mining pref{preference} q{q} q_mine{q_mine} {condition}",
        )
    ...
    def reserve_ship_by_q(self, ship, q, forced=False, depend_on=None, priority=None):
        ...
        self.reserving_ships[ship.id] = [priority, ship.id, q, ship, forced, depend_on]
        ...
        while u:
            if self.determined_ships.get(u.id, None) is None:
                self.log(
                    loglevel="warning",
                    s=f"{t.id} depend on {u.id}, but {u.id} was not determined",
                )
                break
    ...
    def resolve_ship_by_q(self, ship, q, forced=False, depend_on=None):
        ...
        if depend_on and len(q.shape) == 2:
            ...
            if determined is None or determined == "reserved":
                self.log(
                    loglevel="warning",
                    s=f"reserve_ship_by_q s{ship.id} depend_on{depend_on.id} determined_depend_on={determined}",
                )
                q = q[I_MINE]
            else:
        ...
        for i_action, q_i in sorted(zip(i_actions, q), key=itemgetter(1), reverse=True):
            ...
            cell = cells[i_action]
            m = self.flags.get(
                I_FLAG_NEXT_SHIP_POSITION, x=cell.position[0], y=cell.position[1]
            )
            if forced or (not m):
    ...
    def calculate_escape_preference(self, ship):
        ...
        maybe_killed = np.zeros(
            LEN_MOVE, dtype=np.int32
        )  # この ship が opponent にkillされるかも
        maybe_kill_opponent = np.zeros(
            (LEN_MOVE, PLAYERS), dtype=np.int32
        )  # opponent視点 他のopponentもkillできるので一石二鳥
        maybe_cancel_opponent = np.zeros(
            (LEN_MOVE, PLAYERS), dtype=np.int32
        )  # opponent視点 opponent同士で相殺発生
        maybe_killed_by_opponent = np.zeros(
            (LEN_MOVE, PLAYERS), dtype=np.int32
        )  # opponent視点 他のopponentにkillされるので行きたくない
        maybe_kill_ally = np.zeros(
            (LEN_MOVE, PLAYERS), dtype=np.int32
        )  # opponent視点 他のallyもkillできるので一石二鳥
        maybe_cancel_ally = np.zeros(
            (LEN_MOVE, PLAYERS), dtype=np.int32
        )  # opponent視点 味方が相殺できる
        maybe_killed_by_ally = np.zeros(
            (LEN_MOVE, PLAYERS), dtype=np.int32
        )  # opponent視点 味方がkillできる
        ...
        for k_action, cell_k in enumerate(neighbor_cells):
            ...
            if cell_k.shipyard and cell_k.shipyard.player_id != self.player_id:
                ...
                q[k_action] *= 0.1
            i_k, j_k = position_to_ij(cell_k.position)
            detour_advantage_k = int(
                1e-6 + self.scores[I_SCORE_DETOUR_ADVANTAGE, i_k, j_k]
            )
        ...
        for k_action, cell_k in enumerate(neighbor_cells):
            for l_action, cell_l in enumerate(self.neighbor_cells(cell_k)):
                ...
                if not ship_l:
                    ...
                    min_ally_halite[k_action] = min(
                        min_ally_halite[k_action], ship_l.halite
                    )
                else:
                    ...
                    non_stop_chance_l = (
                        l_action == 0
                        and ship_l.halite == 0
                        and has_ground_halite[k_action]
                    )
                    non_stop_threat_k = (
                        k_action == 0
                        and ship_l.halite == 0
                        and has_ground_halite[l_action]
                    )
                    min_opponent_halite[k_action, ship_l.player_id] = min(
                        min_opponent_halite[k_action, ship_l.player_id], ship_l.halite
                    )
                    if ship_l.halite < ship.halite:  # 択状況に応じて期待値を分配
                        ...
                        self.log(
                            ...
                            step=self.board.step,
                            id_=ship.id,
                            s=f"  k{k_action} l{l_action} sl{ship_l.id}{ship_l.position} h{ship_l.halite} nscl{non_stop_chance_l} nstk{non_stop_threat_k} count{count:.2f}",
                        )
            ...
            for opponent_id in oids:
                ...
                if 99999 <= halite:  # そもそもいない
                    continue
                killed_count = (
                    maybe_killed_by_opponent[k_action, opponent_id]
                    + maybe_killed_by_ally[k_action, opponent_id]
                )
                cancel_count = (
                    maybe_cancel_opponent[k_action, opponent_id]
                    + maybe_cancel_ally[k_action, opponent_id]
                )
                kill_count = (
                    maybe_kill_opponent[k_action, opponent_id]
                    + maybe_kill_ally[k_action, opponent_id]
                )
                self.log(
                    indent=2,
                    step=self.board.step,
                    id_=ship.id,
                    s=f"k{k_action} oid{opponent_id} h{halite} killed{killed_count} cancel{cancel_count} kill{kill_count}",
                )
                ...
                if 0 < kill_count:  # このship以外にも複数ターゲットある
                    ...
                    self.log(
                        loglevel="warning",
                        s=f"s{ship.id} h{ship.halite} calculate_evade_preference: unreachable code: k_action{k_action} opponent_id{opponent_id} counts[{killed_count},{cancel_count},{kill_count}]",
                    )
    ...
    def ship_on_shipyard_strategy(self, ship, q=None):
        self.log(
            step=self.board.step,
            id_=ship.id,
            s=f"h{ship.halite} ship_on_shipyard_strategy",
        )
        if q is None:
            q = np.ones(LEN_MOVE, dtype=np.float32)
    ...
    def empty_ship_strategy(self, ship, q=None, mine_threshold=1000.0):
        min_i = 0
        if q is None:
        ...
        (
            ...
            nearest_shipyard,
            nearest_shipyard_distance,
        ) = self.returning_preference(ship)
        if nearest_shipyard:
        ...
        for i_action, cell in enumerate(
            ...
            if cell.shipyard:
                ...
                if player_id == self.player_id:
                    ...
                    n0, n1 = self.opponent_history[player_id].get(
                        "defense_against_shipyard_attack", [0, 0]
                    )
            ...
            score = (
                self.scores[I_SCORE_EMPTY_ALLY_D4, i, j]
                + self.scores[I_SCORE_NON_EMPTY_ALLY_D4, i, j]
                - my_empty_score_d4
            )
            ...
            if 0.5 < score:
                ...
                if (
                    nearest_shipyard
                    and calculate_distance(nearest_shipyard_position, cell.position) < 2
                ):
                    # shipyard圏内
        ...
        self.log(
            step=self.board.step,
            id_=ship.id,
            s=f"h{ship.halite} empty_ship_strategy q{q} cond=({condition})",
        )
    ...
    def ship_strategy(self, ship, q=None):
        ...
        if 0 < ship.halite:
            self.log(
                step=self.board.step,
                id_=ship.id,
                s=f"h{ship.halite} mining_ship_strategy",
            )
    ...
    def shipyard_strategy(self):
        ...
        len_ships = len(self.board.current_player.ships)
        len_shipyards = len(self.board.current_player.shipyards)
        for shipyard, score, i, j in sorted(
            to_sort, key=itemgetter(1, 2, 3), reverse=True
        ):
    ...
    def reserve_shipyard(self, shipyard, next_action, forced=False):
        ...
        if next_action == ShipyardAction.SPAWN:
            if (not forced) and self.flags.get(
                I_FLAG_NEXT_SHIP_POSITION,
                x=shipyard.position[0],
                y=shipyard.position[1],
            ):
                self.log(
                    loglevel="warning",
                    s=f"yd{shipyard.id} reserve_shipyard: try to spawn although merging",
                )
    ...
    def __call__(self, board):
        ...
        self.update_score()
        self.log(
            f"world_halite={self.world_halite}, halite_per_ship={self.halite_per_ship}, free_halite={self.free_halite}"
        )
        ...
        self.log(
            step=self.board.step,
            s=f"len(projects)={len(self.projects)}, len(sorted_projects)={len(sorted_projects)}",
        )
        ...
        for priority, ship_id, q, ship, forced, depend_on in reserving_ships:
            priority_depend_on = (
                ...
                else self.calculate_collision_priority(depend_on)
            )
            self.log(
                id_=ship_id,
                s=f"{ship.position} resolve_ship prio{priority} h{ship.halite} threat{self.collision_threat.get(ship_id, None)} q{q}, dep{depend_on.id if depend_on else None} prio{priority_depend_on}",
            )
        for priority, ship_id, q, ship, forced, depend_on in reserving_ships:
